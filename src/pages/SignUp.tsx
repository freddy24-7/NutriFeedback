...
const OtpSchema = z.object({ code: z.string().min(6, 'Required') });
...
export function SignUpPage() {
  ...
  // Clear OTP field after browser autofill kicks in
  useEffect(() => {
    if (verifying) {
      const t = setTimeout(() => setOtpValue('code', ''), 50);
      return () => clearTimeout(t);
    }
  }, [verifying, setOtpValue]);
  ...
  const onSubmit = async (data: SignUpFormInput) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        if (token) await provisionUser(token);
      ...
      if (code === 'form_identifier_exists') {
        setServerError(t('auth.error.emailTaken'));
      } else {
        setServerError(t('auth.error.generic'));
      }
    }
  ...
  const onVerify = async (data: OtpInput) => {
    if (!isLoaded) return;
    setServerError(null);
    try {
    ...
    return (
      <>
        ...
        <p className="mt-1 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
          {t('auth.signUp.confirmEmailMessage', { email: submittedEmail })}
        </p>

        <form
          onSubmit={(e) => void handleOtpSubmit(onVerify)(e)}
        ...
        >
          <div>
            ...
            <input
              ...
              aria-invalid={otpErrors.code !== undefined}
              className={cn(
              ...
              style={{
                ...
                borderColor: otpErrors.code ? 'var(--color-error)' : 'var(--color-border)',
          ...
          >
            {isOtpSubmitting ? t('common.loading') : t('auth.confirm.submit')}
          </button>
        </form>
      </>
    );
